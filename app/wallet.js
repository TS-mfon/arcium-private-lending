...
async function connectWallet() {
  ...
  document
    .querySelectorAll("[data-wallet-full]")
    .forEach((n) => (n.textContent = k || "Not connected"));
...
async function hydrateDeployment() {
  try {
    ...
    document
      ...
      .forEach((n) => (n.textContent = d.programId || "No program id"));
  } catch {
    document.querySelectorAll("[data-deployment-status]").forEach((n) => {
    ...
    document
      .querySelectorAll("[data-program-id]")
      .forEach((n) => (n.textContent = "No verified deployment metadata"));
  }
}
document
